...
if [ "x$alg" == "xnaivebayes"  -o  "x$alg" == "xcnaivebayes" ]; then
  ...
  mkdir ${WORK_DIR}/water-all
  cp -R ${APP_DIR}/repo/test/* ${WORK_DIR}/water-all
  ...
  ./bin/mahout testnb \
    ...
    -ow -o ${WORK_DIR}/water-testing $c 2>&1 | tee $OUTPUT
...
#
NUM=$(cat $OUTPUT | grep Accuracy | awk '{print $2}' | cut -d '%' -f 1)
echo $NUM >> $OUTPUT
